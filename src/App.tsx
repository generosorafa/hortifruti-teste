...
function OrdersPage({ orders, saved, startNewOrder, editOrder, updatePayment, updatePaymentMethod, updatePaymentReference, company, externalQuery = "" }: { orders: Order[]; saved: boolean; startNewOrder: () => void; editOrder: (order: Order) => void; updatePayment: (number: string, status: PaymentStatus) => void; updatePaymentMethod: (number: string, method: PaymentMethod) => void; updatePaymentReference: (number: string, reference: string) => void; company: CompanyProfile; externalQuery?: string }) {
