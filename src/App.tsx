...
const selectableProductUnits = (current?: Unit): Unit[] => current === "CX" ? [...productUnits, "CX"] : [...productUnits];
  ...
  return (
    <>
      ...
      {type === "products" && bulkOpen && <section className="panel bulk-import">
        <div className="registry-editor__heading"><div><strong>Inserir vários produtos</strong><span>Use uma linha por produto: Nome, Número, Categoria, Unidade, Custo, Venda. Unidades: SC, KG, UN, FD, MÇ, BDJ, PCT ou CU.</span></div><button className="icon-button" aria-label="Fechar importação" onClick={() => setBulkOpen(false)}><X size={18} /></button></div>
      ...
      {editing && <section className="panel registry-editor">
        ...
        <div className="registry-editor__grid">
          ...
          {type === "products" ? <>
            ...
            <label className="registry-field">Unidade<select value={editing.unit} onChange={(event) => updateDraft("unit", event.target.value as Unit)}>{selectableProductUnits(editing.unit).map((unit) => <option value={unit} key={unit}>{unit}</option>)}</select></label>
